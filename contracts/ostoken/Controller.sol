...
contract Controller {
  ...
  function withdraw(address vault, address receiver, uint256 shares) external override {
    ...
    // reduce number of supplied shares
    ...
    uint256 healthFactor = _getHealthFactor(
      getSuppliedAssets(msg.sender),
      osToken.convertToAssets(_borrowedShares[msg.sender])
    );
    if (healthFactor <= _healthFactorLiqThreshold) revert LowHealthFactor();
  ...
  function borrow(
    ...
    uint256 healthFactor = _getHealthFactor(suppliedAssets, borrowedAssets);
    if (healthFactor <= _healthFactorLiqThreshold) revert LowHealthFactor();
  ...
  function _getHealthFactor(
    ...
    uint256 liqAssets = Math.mulDiv(suppliedAssets, liqThresholdPercent, _maxPercent);
    return Math.mulDiv(liqAssets, _wad, borrowedAssets);
  }
}
