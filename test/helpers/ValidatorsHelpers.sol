...
abstract contract ValidatorsHelpers is Test, KeeperHelpers {
    function _registerValidator(
        ...
        IKeeperValidators.ApprovalParams memory approvalParams =
            _getValidatorsApproval(keeper, validatorsRegistry, vault, "ipfsHash", deposits, isV1Validator);
    ...
    function _extractBytes(bytes memory data, uint256 offset, uint256 length) internal pure returns (bytes memory) {
    ...
    function _collateralizeVault(address keeper, address validatorsRegistry, address vault) internal {
        ...
        IKeeperValidators.ApprovalParams memory approvalParams =
            _getValidatorsApproval(keeper, validatorsRegistry, vault, "ipfsHash", depositAmounts, false);
    ...
    function _getValidatorDepositData(
        ...
        return isV1Validator
            ? bytes.concat(publicKey, signature, depositDataRoot)
            : bytes.concat(publicKey, signature, depositDataRoot, bytes8(SafeCast.toUint64(depositAmount)));
    }
    ...
    function _getDepositDataRoot(
        ...
        bytes32 signatureRoot = sha256(abi.encodePacked(sha256(signatureFirstHalf), sha256(signatureSecondHalf)));
        return sha256(
            abi.encodePacked(
