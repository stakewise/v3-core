...
contract VaultOsTokenTest is Test, EthHelpers {
    ...
    function test_mintOsToken_maxAmount() public {
        ...
        assertGt(assets, 0, "Should have minted assets");
        assertGt(vault.osTokenPositions(owner), initialOsTokenShares, "Owner's OsToken position should increase");
    ...
    function test_burnOsToken_basic() public {
        ...
        assertEq(remainingPosition, initialPosition - burnAmount, "Position should be reduced by burn amount");
    ...
    function test_burnOsToken_multipleBurns() public {
        ...
        assertEq(positionAfterFirstBurn, initialPosition - burnAmount1, "Position incorrect after first burn");
        ...
        assertEq(
            positionAfterSecondBurn, initialPosition - burnAmount1 - burnAmount2, "Position incorrect after second burn"
        );
    ...
    function test_redeemOsToken_onlyRedeemer() public {
        ...
        assertLt(vault.osTokenPositions(owner), mintAmount, "Position should be reduced after redemption");
    }
    ...
    function test_redeemOsToken_basic() public {
        ...
        assertEq(finalPosition, initialPosition - redeemAmount, "Position should be reduced by redemption amount");
    ...
    function test_redeemVsLiquidate() public {
        ...
        assertLt(vault.osTokenPositions(owner), maxOsTokenShares, "Position should be reduced after redemption");
    }
    ...
    function test_liquidateOsToken_basic() public {
        ...
        IOsTokenConfig.Config memory config = osTokenConfig.getConfig(address(vault));
        int256 requiredPenalty =
            int256(vault.totalAssets()) - int256((vault.totalAssets() * config.liqThresholdPercent) / 1e18);
        requiredPenalty = -requiredPenalty;

        // Apply the penalty
        IKeeperRewards.HarvestParams memory harvestParams =
            _setEthVaultReward(address(vault), int160(requiredPenalty), 0);
        vault.updateState(harvestParams);
        ...
        assertApproxEqAbs(
            vault.osTokenPositions(owner), 0, 0.0001 ether, "Position should be reduced after liquidation"
        );
    ...
    function test_liquidateOsToken_bonus() public {
        ...
        assertApproxEqAbs(
            receivedAssets, expectedAssets, tolerance, "Received assets should match expected bonus calculation"
        );
    }
    ...
    function test_liquidateOsToken_liquidationDisabled() public {
        ...
        address vaultAddr = _getOrCreateVault(VaultType.EthVault, adminWithDisabledLiq, initParams, false);
        ...
        // Create a position
        vm.prank(vaultOwner);
        uint256 osTokenAssets = vaultWithDisabledLiq.mintOsToken(vaultOwner, type(uint256).max, referrer);
        uint256 osTokenShares = osTokenVaultController.convertToShares(osTokenAssets);

        // Apply severe penalty
        IKeeperRewards.HarvestParams memory harvestParams =
            _setEthVaultReward(address(vaultWithDisabledLiq), int160(int256(-10 ether)), 0);
    ...
    function test_liquidateOsToken_partialLiquidation() public {
        ...
        assertApproxEqAbs(
            vault.osTokenPositions(owner), 0, 0.0001 ether, "Position should be zero after complete liquidation"
        );
    }
    ...
    function test_transferOsTokenPositionToEscrow_basic() public {
        ...
        (address escrowOwner, uint256 exitedAssets, uint256 escrowOsTokenShares) =
        ...
        assertEq(ownerBalanceAfter - ownerBalanceBefore, claimedAssets, "Incorrect amount of assets transferred");
    ...
    function test_transferOsTokenPositionToEscrow_partialTransfer() public {
        ...
        // Verify position in escrow
        (address escrowOwner,, uint256 escrowOsTokenShares) =
            contracts.osTokenVaultEscrow.getPosition(address(vault), exitPositionTicket);
