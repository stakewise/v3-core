...
contract OsTokenTest is Test, EthHelpers {
  ...
  function test_controllerIntegration() public {
    ...
    assertEq(
      osToken.balanceOf(user),
      initialBalance,
      'Balance should decrease back to initial amount'
    );
  }
  ...
  function test_permit() public {
    ...
    bytes32 structHash = keccak256(
      abi.encode(permitTypehash, tokenOwner, user, value, nonceBefore, deadline)
    );
  ...
  function test_permit_invalidSignature() public {
    ...
    bytes32 structHash = keccak256(
      abi.encode(permitTypehash, tokenOwner, user, value, nonce, deadline)
    );
  ...
  function test_erc20_transfers() public {
    ...
    assertEq(
      osToken.balanceOf(recipient),
      transferAmount + 5 ether,
      'Recipient should receive additional tokens'
    );
    assertEq(
      osToken.balanceOf(user),
      amount - transferAmount - 5 ether,
      "User's balance should be further reduced"
    );
    assertEq(
      osToken.allowance(user, spender),
      approvalAmount - 5 ether,
