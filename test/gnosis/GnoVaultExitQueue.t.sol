...
contract GnoVaultExitQueueTest is Test, GnoHelpers {
  ...
  function testVaultV2ToV3ExitQueue() public {
    ...
    (uint256 leftTickets1, , uint256 exitedAssets1) = vault.calculateExitedAssets(
      user1,
      positionTicket1,
      timestamp1,
    ...
    (uint256 leftTickets2, , uint256 exitedAssets2) = vault.calculateExitedAssets(
      user2,
      positionTicket2,
      timestamp2,
    ...
    assertEq(
      user2GnoAfterClaim,
      user2InitialGno + exitedAssets2Claimed,
      'User2 received incorrect amount of GNO tokens'
    );
    ...
    assertApproxEqAbs(
      user1GnoAfterClaim,
      user1InitialGno + exitedAssets1,
      1,
      'User1 received incorrect amount of GNO tokens'
    );
    ...
    (, , uint256 exitedAssets3) = vault.calculateExitedAssets(
      user3,
      positionTicket3,
      timestamp3,
      uint256(exitQueueIndex3)
    );
    ...
    assertApproxEqAbs(
      user3GnoAfterClaim,
      user3InitialGno + exitedAssets3,
      1,
      'User3 received incorrect amount of GNO tokens'
    );
    ...
    (, , uint256 exitedAssets4) = vault.calculateExitedAssets(
      user2,
      positionTicket4,
      timestamp4,
      uint256(exitQueueIndex4)
    );
    ...
    assertEq(
      vault.convertToAssets(vault.getShares(user1)),
      depositAmount - exitAmount,
      'User1 assets incorrect'
    );
    assertEq(vault.getShares(user2), 0, 'User2 should have 0 shares');
    assertEq(
      vault.convertToAssets(vault.getShares(user3)),
      depositAmount - exitAmount,
      'User3 assets incorrect'
    );
  }
