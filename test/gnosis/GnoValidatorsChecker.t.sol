...
contract GnoValidatorsCheckerTest is Test, GnoHelpers {
    ...
    function testGetExitQueueFunctions_AfterStateUpdate() public {
        ...
        uint256 initialCumulativeTickets = validatorsChecker.getExitQueueCumulativeTickets(prevVersionVault);
        ...
        IKeeperRewards.HarvestParams memory harvestParams =
            _setGnoVaultReward(prevVersionVault, int160(int256(0)), uint160(0));
        validatorsChecker.updateVaultState(prevVersionVault, harvestParams);

        // Get exit queue state after update
        uint256 updatedCumulativeTickets = validatorsChecker.getExitQueueCumulativeTickets(prevVersionVault);
        ...
        assertTrue(
            updatedMissingAssets <= initialMissingAssets, "Missing assets should not increase after state update"
        );
    }
}
