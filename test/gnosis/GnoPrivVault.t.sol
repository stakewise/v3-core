...
contract GnoPrivVaultTest is Test, GnoHelpers {
    ...
    function test_upgradesCorrectly() public {
        ...
        assertEq(contracts.gnoToken.allowance(address(privVault), address(contracts.validatorsRegistry)), 0);
        ...
        assertEq(
            contracts.gnoToken.allowance(address(privVault), address(contracts.validatorsRegistry)), type(uint256).max
        );
