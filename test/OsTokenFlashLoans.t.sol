...
contract OsTokenFlashLoansTest is Test, EthHelpers {
    ...
    function test_flashLoan_success() public {
        ...
        assertEq(flashLoansPostBalance, flashLoansPreBalance, "Flash loans contract balance should remain the same");

        // Recipient's balance should be reduced by the loan amount
        // (It repays the loan from its own balance)
        assertEq(
            recipientPostBalance, recipientPreBalance, "Recipient should have the same balance after repaying the loan"
        );
    }
