...
contract OsTokenConfigTest is Test, EthHelpers {
  ...
  function test_initialState() public view {
    ...
    assertTrue(
      config.ltvPercent <= MAX_PERCENT,
      'Default LTV should be less than or equal to max percent'
    );

    // Check that liqThresholdPercent is either valid or disabled
    if (config.liqThresholdPercent != DISABLED_LIQ_THRESHOLD) {
      assertTrue(
        config.liqThresholdPercent > config.ltvPercent,
        'Default liq threshold should be greater than LTV'
      );
      assertTrue(
        config.liqThresholdPercent < MAX_PERCENT,
        'Default liq threshold should be less than max percent'
      );
    }
    ...
    if (config.liqThresholdPercent == DISABLED_LIQ_THRESHOLD) {
      ...
      assertTrue(
        config.liqBonusPercent >= MAX_PERCENT,
        'Default liq bonus should be at least max percent'
      );
  ...
  function test_updateConfig_forVault() public {
    ...
    assertEq(
      ...
      newConfig.liqBonusPercent,
      'Vault liquidation bonus not updated correctly'
    );
    ...
    assertNotEq(
      config.liqBonusPercent,
      defaultConfig.liqBonusPercent,
      'Default liquidation bonus should not be changed'
    );
  }
  ...
  function test_updateConfig_disabledLiquidations() public {
    ...
    assertEq(
      config.liqThresholdPercent,
      uint64(DISABLED_LIQ_THRESHOLD),
      'Vault liquidation threshold should be disabled'
    );
  ...
  function test_getConfig_defaultFallback() public view {
    ...
    assertEq(
      config.liqThresholdPercent,
      defaultConfig.liqThresholdPercent,
      'Should return default liquidation threshold'
    );
    assertEq(
      config.liqBonusPercent,
      defaultConfig.liqBonusPercent,
