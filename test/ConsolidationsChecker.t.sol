...
contract ConsolidationsCheckerTest is Test, EthHelpers {
  ...
  function setUp() public {
    ...
    for (uint i = 0; i < 4; i++) {
      (_oracleAddresses[i], _oraclePrivateKeys[i]) = makeAddrAndKey(
        string(abi.encodePacked('oracle', vm.toString(i)))
      );
    }
  ...
  function _getMessageHash(
    ...
    bytes32 structHash = keccak256(
      abi.encode(_consolidationsCheckerTypeHash, _vault, keccak256(validators))
    );
  ...
  function _generateValidSignatures(
    address _vault,
    bytes memory validators,
    uint numSigners
  ...
  function test_verifySignatures_success() public {
    ...
    bytes memory validSignatures = _generateValidSignatures(
      vault,
      validatorsData,
      contracts.keeper.validatorsMinOracles()
    );
  ...
  function test_verifySignatures_tooFewSignatures() public {
    ...
    // Also test the direct isValidSignatures function
    bool isValid = consolidationsChecker.isValidSignatures(
      vault,
      validatorsData,
  ...
  function test_verifySignatures_exactMinimumSignatures() public {
    ...
    bytes memory signatures = _generateValidSignatures(
      vault,
      validatorsData,
      contracts.keeper.validatorsMinOracles()
    );
  ...
  function test_verifySignatures_moreThanMinimumSignatures() public {
    ...
    bytes memory signatures = _generateValidSignatures(
      vault,
      validatorsData,
      contracts.keeper.validatorsMinOracles() + 1
    );
