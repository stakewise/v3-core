...
contract EthGenesisVaultTest is Test, EthHelpers {
    ...
    function test_migrate_failsWithInvalidPoolEscrowOwner() public {
        ...
        vm.mockCall(poolEscrow, abi.encodeWithSelector(IEthPoolEscrow.owner.selector), abi.encode(address(0x123)));
    ...
    function test_fallback_acceptsEtherFromPoolEscrow() public {
        ...
        assertEq(vault.totalShares(), totalShares, "Total shares should not change after deposit from pool escrow");
        assertEq(address(vault).balance, initialBalance + amount, "Vault balance should increase by transfer amount");
    }
    ...
    function test_claimsPoolEscrowAssets() public {
        ...
        (uint128 queuedShares,,, uint128 totalExitingAssets,) = vault.getExitQueueData();
        uint256 vaultAmount = address(vault).balance + vault.convertToAssets(queuedShares) + totalExitingAssets;
