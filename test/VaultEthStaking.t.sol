...
contract VaultEthStakingTest is Test, EthHelpers {
    ...
    function test_invalidSecurityDeposit() public {
        ...
        assertTrue(address(newVault) != address(0), "Vault should be created with valid security deposit");
    }
    ...
    function test_deposit() public {
        ...
        assertEq(sender.balance, senderInitialBalance - depositAmount, "Sender balance should decrease");
        assertEq(address(vault).balance, vaultInitialBalance + depositAmount, "Vault balance should increase");
        ...
        assertApproxEqAbs(shares, expectedShares, 1, "Shares should match the expected conversion rate");

        // Verify totalAssets and totalShares updated
        assertEq(vault.totalAssets(), vaultTotalAssetsBefore + depositAmount, "Total assets should increase");
    ...
    function test_withdrawableAssets() public {
        ...
        assertGe(
            withdrawable, withdrawableBefore + depositAmount, "Withdrawable assets should include deposited amount"
        );
    }
    ...
    function test_vaultAssets() public {
        ...
        assertEq(vault.totalAssets(), initialAssets + depositAmount, "Total assets should increase by deposit amount");
    ...
    function test_registerValidators_succeeds() public {
        ...
        IKeeperValidators.ApprovalParams memory approvalParams =
            _getEthValidatorApproval(address(vault), 32 ether, "ipfsHash", true);
    ...
    function test_receive() public {
        ...
        assertEq(vault.totalAssets(), totalAssetsBefore + sendAmount, "Vault total assets didn't increase correctly");
        assertApproxEqAbs(
            vault.getShares(sender), depositShares + userSharesBefore, 1, "User should have deposit amount"
        );
    }
    ...
    function test_updateStateAndDeposit() public {
        ...
        uint256 shares = vault.updateStateAndDeposit{value: depositAmount}(receiver, referrer, harvestParams);
        _stopSnapshotGas();

        // Verify the deposit was successful and state was updated
        assertGt(shares, 0, "Should have minted shares");
        assertEq(
            address(vault).balance, initialBalance + depositAmount, "Vault balance should increase by deposit amount"
        );
    ...
    function test_receiveFromMevEscrow() public {
        ...
        assertEq(address(vault).balance, initialBalance + mevAmount, "Vault balance should increase by MEV amount");
    }
    ...
    function test_depositAndMintOsToken() public {
        ...
        uint256 assets = vault.depositAndMintOsToken{value: depositAmount}(sender, type(uint256).max, referrer);
        _stopSnapshotGas();
    ...
    function test_updateStateAndDepositAndMintOsToken() public {
        ...
        uint256 assets = vault.updateStateAndDepositAndMintOsToken{value: depositAmount}(
            sender, type(uint256).max, referrer, harvestParams
        );
        _stopSnapshotGas();
    ...
    function test_transferVaultAssets() public {
        ...
        uint256 senderDeposit = vault.convertToAssets(queuedShares) + totalExitingAssets + depositAmount;
    ...
    function test_harvestAssets() public {
        ...
        IKeeperRewards.HarvestParams memory harvestParams =
            _setEthVaultReward(address(vault), int160(int256(0.3 ether)), mevReward);
        ...
        assertGt(address(vault).balance, vaultInitialBalance, "Vault balance should increase from MEV rewards");
    }
